defmodule SFTPToolkit.Upload do
  ...
  * `{:remote_close, info}` - the `:ssh_sftp.close/2` on the remote file 
  ...
  defp do_upload_file(sftp_channel_pid, local_path, remote_path, chunk_size, operation_timeout) do
    case IO.binread(local_path, chunk_size) do
      :eof ->
        :ok
      {:error, reason} ->
        {:error, {:read, reason}}
      data ->
        case :ssh_sftp.write(sftp_channel_pid, remote_path, data, operation_timeout) do
          :ok ->
            do_upload_file(sftp_channel_pid, local_path, remote_path, chunk_size, operation_timeout)
          {:error, reason} ->
            {:error, {:write, reason}}
        end
    end
